...
namespace logic {
    ...
    PacManModel::PacManModel(double x, double y, double width, double height)
            : EntityModel(x, y, width, height),
              ...
              m_nextDirection(Direction::STOP),
              m_speed(0.35f)
    {
    ...
    void PacManModel::commitDirection() {
    ...
    std::pair<double, double> PacManModel::getPositionAt(float dt, Direction dir) const {
        ...
        switch (dir) {
            case Direction::UP:    nextY -= delta; break;
            case Direction::DOWN:  nextY += delta; break;
            case Direction::LEFT:  nextX -= delta; break;
            case Direction::RIGHT: nextX += delta; break;
            case Direction::STOP:  break;
        }
        return {nextX, nextY};
    }

    void PacManModel::accept(Visitor& visitor) {
        visitor.visit(*this);
    }
