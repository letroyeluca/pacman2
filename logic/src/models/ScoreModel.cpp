...
namespace logic {
    ...
    ScoreModel::ScoreModel(double x, double y, double size)
            : EntityModel(x, y, size, size), // Geef x, y door. Width/Height (1.0) maakt niet uit voor tekst.
              m_currentScore(0),
              m_highScore(0),
    ...
    void ScoreModel::onNotify(const Subject& subject, Event event) {
        // Check welk event er gebeurt. De 'subject' parameter vertelt ons WIE het stuurde
        // (bijv. de specifieke Coin), maar dat hebben we voor de score niet per se nodig.
        ...
        if (event == Event::CoinEaten) {
            ...
            int points = 10;
            if (timeDiff < 0.5f) points += 20;
            else if (timeDiff < 1.0f) points += 10;
