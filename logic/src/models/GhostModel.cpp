...
#include <cmath> // Nodig voor std::round en std::abs
#include <iostream>

namespace logic {

GhostModel::GhostModel(double x, double y, double width, double height, char type)
    : EntityModel(x, y, width, height), m_direction(Direction::UP), m_nextDirection(Direction::UP), m_speed(0.25f),
      m_type(type), m_locked(false), m_startX(x), m_startY(y), m_isDead(false), m_hasExitedHouse(false) {
    setHitboxScale(0.2);

    // Initialisatie strategieën (gebruikt unique_ptr, dit is goed)
    if (type == 'R') {
        setSpawnDelay(0.0f);
...
// ... (frighten, setSpawnDelay, die, reset blijven hetzelfde) ...
void GhostModel::frighten(float duration) {
    if (m_isDead || !m_hasExitedHouse || !m_isActive)
        return;
...
void GhostModel::think(World& world) {
    ...
    if (m_isActive && !m_hasExitedHouse) {
        ...
        if (m_y <= exitY) {
            ...
            // ... (Zelfde exit logica als voorheen) ...
    ...
    // -----------------------------------------------------------
    // STAP B: STRATEGIE (AANGEPAST: Geen raw pointers!)
    // -----------------------------------------------------------

    // VERANDERING: Gebruik een Reference (&) naar de strategie in de unique_ptr.
    // Dit is veilig omdat de unique_ptr eigenaar blijft en we de strategie niet verwijderen.
    GhostStrategy& activeStrategy = m_isFrightened ? *m_frightenedStrategy : *m_normalStrategy;

    // Omdat activeStrategy een referentie is, gebruiken we punt (.) i.p.v. pijl (->)
    Direction newDir = activeStrategy.calculateNextMove(*this, world);
    queueDirection(newDir);
}

// ... (Rest van de functies: queueDirection, commitDirection, etc. blijven hetzelfde) ...
...
void GhostModel::update(float dt) {
    // ... (Update logica blijft exact hetzelfde, geen pointers hier) ...
    // 1. SPAWN TIMER
    if (!m_isActive) {
        m_spawnTimer -= dt;
