...
// ------------------------------------------------------------------
    ...
    public:
        Direction calculateNextMove(GhostModel& ghost, World& world) override {
            ...
            if (!pacman) return Direction::STOP;
            ...
            if (options.empty()) return Direction::STOP;
            ...
            // STAP D: Tie-breaking
            // ----------------------------------------------------------
            if (bestMoves.empty()) return Direction::STOP;
            ...
            if (index >= bestMoves.size()) index = static_cast<int>(bestMoves.size()) - 1;
    ...
    public:
        Direction calculateNextMove(GhostModel& ghost, World& world) override {
            ...
            switch (pDir) {
                ...
                default: break; // Bij STOP blijven we op Pac-Man zelf richten
            }
            ...
            // ----------------------------------------------------------
            if (bestMoves.empty()) return Direction::STOP; // Should not happen
...
// ------------------------------------------------------------------
    ...
    public:
        Direction calculateNextMove(GhostModel& ghost, World& world) override {
            ...
            // --- GATE LOGICA ---
            if (world.isGateAt(gx, gy)) { freeDOWN = false; }       // We staan op een gate
            if (world.isGateAt(gx, gy + ts)) { freeDOWN = false; }  // Tegel onder ons is een gate
            ...
            if (options.empty()) return Direction::STOP;
            if (options.size() == 1) return options[0]; // Geen keuze (bocht of gang)
            ...
            if (!shouldSwitch && canKeepGoing) {
                ...
                if (index >= options.size()) index = static_cast<int>(options.size()) - 1;
